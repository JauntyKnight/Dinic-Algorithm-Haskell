...
fromEdgeList edges = fromAssocList assocl
  where
    grouped =
      groupBy (\(u, _, _) (v, _, _) -> u == v) $
      sortBy (\(u, _, _) (v, _, _) -> compare u v) edges
    fst3 (x, _, _) = x
    assocl =
      [ (v, nbs)
      | group <- grouped
      , let v = fst3 $ head group
      , let nbs = map (\(_, w, c) -> (w, c)) group
      ]
...
pushFlow g u v f = Map.adjust (Map.adjust (subtract f) v) u g'
  where
...
constructResidual g =
  fromEdgeList $ edges ++ map (\(u, v, c) -> (v, u, 0)) edges
  where
    edges = toEdgeList g
